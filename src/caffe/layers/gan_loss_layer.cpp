...
void GANLossLayer<Dtype>::LayerSetUp(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
      ...
      iter_idx_ = 0;
      dis_mod_ = this->layer_param_.gan_loss_param().dis_mod();
      gen_mod_ = this->layer_param_.gan_loss_param().gen_mod();
      //discriminative mode
      if (bottom.size() == 2) {
